...
public class AccordionView extends DemoView {
    ...
    private void eventHandling() {
        ...
        addCard("Event handling", accordion);
    }
    ...
    private void complexForm() {
        ...
        // ACCOUNT INFORMATION
        AccordionPanel accountInformation = new AccordionPanel();
        accountInformation.setSummary(createSummary(1, "Account information"));
        accountInformation.addOpenedChangedListener(event -> {
            if (event.isOpened()) progressBar.setValue(25);
        });
        ...
        accountInformation.setContent(accountForm);
        accordion.add(accountInformation);


        // BIO DATA
        AccordionPanel bioData = new AccordionPanel();
        bioData.setSummary(createSummary(2, "Bio data"));
        bioData.addOpenedChangedListener(event -> {
            if (event.isOpened()) progressBar.setValue(50);
        });
        ...
        topicsOfInterest.setSummary(createSummary(3, "Topics of interest"));
        topicsOfInterest.addOpenedChangedListener(event -> {
            if (event.isOpened()) progressBar.setValue(75);
        });
        ...
        AccordionPanel termsAndConditions = new AccordionPanel();
        termsAndConditions.setSummary(createSummary(4, "Terms and conditions"));
        termsAndConditions.addOpenedChangedListener(event -> {
            if (event.isOpened()) progressBar.setValue(100);
        });
        ...
        termsAndConditions.setContent(termsDetails);
        accordion.add(termsAndConditions);
    ...
    private Component createSummary(int index, String title) {
        Div result = new Div();
